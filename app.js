...
function logOrderToLedger(){
  if(!orderList.length||LEDGER_URL.indexOf('PASTE_YOUR')===0)return;
  const summary=orderList.map(i=>`${i.qty}x ${i.name}`).join(', ');
  const total=orderList.reduce((s,i)=>s+i.price*i.qty,0);
  fetch(LEDGER_URL,{method:'POST',mode:'no-cors',headers:{'Content-Type':'text/plain'},body:JSON.stringify({items:summary,total:total})}).catch(()=>{});
}
if(confirmOrder)confirmOrder.addEventListener('click',()=>{logOrderToLedger();cart.classList.remove('open')});
